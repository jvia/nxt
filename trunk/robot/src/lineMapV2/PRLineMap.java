 ...
 *
 */
public class PRLineMap implements RangeMap {
  private Line[] lines;
  ...
  public float range(Pose pose) {
    Line l = new  PRLine(pose.getX(), pose.getY(), pose.getX() + 254f
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      Line tl = new PRLine(pose.getX(), pose.getY(), p.x, p.y);
   ...
   */
  public PRLineMap(Line[] lines, Rectangle boundingRect) {
    this.lines = lines;
  ...
  public boolean inside(Point p) {
    ...
    Line l = new PRLine(p.x, p.y, p.x - boundingRect.width, p.y);
