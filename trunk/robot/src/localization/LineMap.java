 ...
 * 
 */
public class LineMap implements RangeMap, Iterable<Line> {
    ...
    public float range(Pose pose) {
        Line l = new Line(pose.getX(), pose.getY(), pose.getX() + 254f
                                                                  * (float) Math.
                cos(Math.toRadians(pose.getHeading())), pose.getY() + 254f
                                                                      * (float) Math.
                sin(Math.toRadians(pose.getHeading())));
        Line rl = null;
    ...
    public boolean inside(Point p) {
        if (p.x < boundingRect.x || p.y < boundingRect.y)
            return false;
        if (p.x > boundingRect.x + boundingRect.width
            || p.y > boundingRect.y + boundingRect.height)
            return false;
        ...
        int count = 0;
        for (int i = 0; i < lines.length; i++) {
            if (lines[i].intersectsAt(l) != null)
                count++;
        }
    ...
    public void loadMap(DataInputStream dis) throws IOException {
        ...
        boundingRect = new Rectangle(dis.readInt(), dis.readInt(), dis.readInt(), dis.
                readInt());
    }
